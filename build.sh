...
BUILD_VAR="-j$(nproc) O=${OUT_DIR} ARCH=arm64 CC=${CC} REAL_CC=${REAL_CC} CROSS_COMPILE=${CROSS_COMPILE} CLANG_TRIPLE=${CLANG_TRIPLE} LD=ld.lld HOSTLD=ld.lld LLVM=1 LLVM_IAS=1 KCFLAGS=\"-Wno-strict-prototypes -Wno-implicit-int -Wno-return-type -Wno-int-conversion\""
...
build_kernel() {
    ...
    echo "CONFIG_THINLTO=y" >> "${OUT_DIR}/.config"
    echo "# CONFIG_LTO_NONE is not set" >> "${OUT_DIR}/.config"
...
build_dtbo() {
    ...
    DTBO_FILES=$(find "${DTBO_SEARCH_DIR}" -name "sm8150-sec-r5q-overlay-*.dtbo" 2>/dev/null | tr '\n' ' ')
